...
export class Parser {
  ...
  private startNode(type: string, startToken?: Token) {
    ...
    // Fix: Guard against accessing previous() when current is 0
    const prevLine = this.current > 0 ? this.tokens[this.current - 1].line : 1;
    const startLine = token.type === TokenType.EOF ? prevLine : token.line;
  ...
  public parse(): ProgramNode {
    ...
    return {
      type: 'Program',
      id: (this.rootAST as any)?.programId || "UNKNOWN", 
      environmentDivision,
      dataDivision,
      procedureDivision,
  ...
  private parseIdentificationDivision() {
    ...
    this.consume(TokenType.DOT, "End of statement");
    this.endNode();
    if (this.rootAST) (this.rootAST as any).programId = programId; 
  }
          ...
          while (this.check(TokenType.MAP) || (this.check(TokenType.IDENTIFIER) && this.peek().value === 'MAP')) {
              ...
              while (this.match(TokenType.DFHMDF)) {
                   ...
                   if (this.match(TokenType.POS)) {
                       ...
                       this.match(TokenType.COLON) || this.consume(TokenType.IDENTIFIER, ","); 
  ...
  private isKeyword(type: TokenType): boolean {
      return type >= TokenType.MOVE && type <= TokenType.HANDLE; 
  }
}
